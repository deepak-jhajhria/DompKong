...
const FaqSec = () => {
    ...
    return (
        <div>
            <div className="container max-w-[896px] w-full mt-[111px]">
                ...
                <div className="my-[55px]">
                    {accordionData.map((item, index) => (
                        <div className={` ${open === index[0] ? "max-h-24" : "max-h-"} bg-transparent border-b border-[#FDDA60] relative after:absolute after:w-full after:h-[2px] after:left-0 after:bottom-0 after:bg-[#FDDA60] after:blur-sm mt-[25px] `} key={index}>
                            ...
                            <div className={` overflow-hidden duration-300  text-white font-Montserrat font-normal text-xs xs:text-sm sm:text-base mt-[6px] sm:mt-[9px]  ${open === index ? "max-h-24 mx-[15px] mb-5 sm:mb-7 lg:mb-[43px]" : "max-h-0"} ${open === index[0] ? "max-h-24" : "max-h-"}`}>
                                {item.content}
                            </div>
                        </div>))
                    }
